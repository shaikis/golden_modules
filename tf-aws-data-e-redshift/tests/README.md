...
- `kms_key_arn` — from `tf-aws-kms` for cluster encryption (set to `null` for AWS-managed key)

```hcl
# Minimal usage
module "redshift" {
  source = "git::https://github.com/your-org/tf-aws-data-e-redshift.git"
  clusters = {
    "warehouse" = {
