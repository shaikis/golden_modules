...
output "tgw_propagation_default_route_table_id" { value = aws_ec2_transit_gateway.this.propagation_default_route_table_id }
...
output "dx_gateway_association_ids" { value = { for k, v in aws_dx_gateway_association.this : k => v.id } }
output "dx_gateway_association_proposal_ids" { value = { for k, v in aws_dx_gateway_association_proposal.this : k => v.id } }
